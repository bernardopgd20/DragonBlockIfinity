...
void main() {
    ...
    // layers controlável — mais layers = mais bonito mas mais pesado
    for (int i = 0; i < layers; i++) {
        float f = float(i) + 1.0;
        float angularWave = triangleWave(angle * f);
        float layer = triangleWave((Position.y * freqY * f) - (time * speed * f * 0.5));
        totalWave += layer * angularWave / f;
    }
